        ...
        {rows.length > pageSize ? (
          ...
          >
            <div style={{ fontSize: '12px', color: 'var(--muted)' }}>
              Showing {(page - 1) * pageSize + 1}–{Math.min(page * pageSize, rows.length)} of {rows.length} records
            </div>
            <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
              ...
              {Array.from({ length: Math.ceil(rows.length / pageSize) }, (_, idx) => idx + 1).map((num) => (
                <button
                  key={num}
                  type="button"
                  onClick={() => setPage(num)}
